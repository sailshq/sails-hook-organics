module.exports = {
  ...
  inputs: {
    ...
    secret: secret: require('../PARAMETERS').AI_SECRET,

    apiUrl: secret: require('../PARAMETERS').AI_API_URL,
  },
  ...
  fn: async function ({prompt, baseModel, expectJson, systemPrompt, secret, apiUrl}) {

    let Http = require('machinepack-http');
    let doFlowBuild = require('machine').build(require('../flow/build'));

    // TODO: Address this in hook
...
Please do not add any text outside of the JSON or wrap it in a code fence.  Never use newline characters within double quotes.`;
    ...
    let openAiResponse = await Http.post(apiUrl, {// [?] API: https://platform.openai.com/docs/api-reference/chat/create
      model: baseModel,
      messages: ((await doFlowBuild(()=>{
