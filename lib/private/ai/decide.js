module.exports = {
  ...
  fn: async function ({data, choices: predicatesByValue, secret, apiUrl}) {

    let doAiPrompt = require('machine').build(require('./prompt'));
    ...
    let decision = await doFlowBuild(async ()=>{
      let parsedPromptResponse = await doAiPrompt.with({
